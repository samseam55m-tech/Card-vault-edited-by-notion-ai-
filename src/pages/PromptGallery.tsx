...
import { Plus, BookOpen, Trash2, Search, Pin, PinOff, Edit2, X, SlidersHorizontal } from 'lucide-react';
...
export default function PromptGallery() {
  const { promptProjects, addPromptProject, updatePromptProject, loading, vaultEpoch } = useStore();

  // Search, sort, date range and the pinned filter survive navigating into a
  // prompt and back. Keyed by `vaultEpoch` so a cloud restore or sign-out wipe
  // clears them rather than leaving a filter over a vault that no longer
  // matches it. Modals and selection mode stay plain local state.
  const viewKey = `prompts:${vaultEpoch}`;
  ...
  const [filterPinned, setFilterPinned] = useStickyState<'all' | 'pinned' | 'unpinned'>(`${viewKey}:filterPinned`, 'all');
    ...
    <div className="w-full h-full overflow-hidden relative">
        ...
        <div className="flex items-center gap-3">
          <AnimatePresence mode="wait">
            {selectionMode ? (
              ...
              >
                {/* Duplicate page title removed — "Prompt Gallery" is already
                    rendered in the app header breadcrumb above. */}
      ...
      <div
        ...
        style={{
          ...
          // Header offset now derives from the shared --app-header-h token
          // instead of re-deriving `safe-top + 56px` by hand in every page.
